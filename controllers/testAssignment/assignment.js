...
exports.createAssignment = asyncHandler(async (req, res) => {
    const { name, submissionDateTime, description, subjectTimeDetail, subject, batch } = req.body;
    //type : normal | mcq
    const assignmentData = {
        name, submissionDateTime, description, subjectTimeDetail, subject, batch
    }
...
exports.getAllAssignment = asyncHandler(async (req, res) => {
    try {
        let { subjectTimeDetail } = req.query;
        const AssignmentData = await Assignment.find({ subjectTimeDetail });
        return res.status(200).json({ success: true, data: AssignmentData });
    } catch (error) {
        throw new ErrorResponse(`Server error :${error}`, 400);
    }
